...
function StatusFlowCard({ req, onSaved }: { req: Requirement; onSaved?: () => void }) {
  ...
  return <section id="status-flow" className="react-panel react-statusflow-panel">
    ...
    <p className="react-muted">一长条展示需求全流程状态（已完成 / 当前 / 待推进）；相邻状态之间竖排该流转配置的门禁：✓ 已通过、✗ 未通过（会拦住 agent 自动推进）、○ 未校验（人工在面板上改状态跳过门禁，悬停看原因）。点击具体门禁可跳转门禁验证详情页。<strong>点击任意非当前状态节点，弹窗确认后即强制修改状态（via=ui，跳过门禁）</strong>；需求组状态为派生值不可点击。</p>
