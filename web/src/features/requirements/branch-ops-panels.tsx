...
export function ProdMrPanel({ req, round }: { req: Requirement; round: number }) {
  ...
  const createMrs = async () => {
    ...
    setError(null)
    try {
      // round 与代码差异卡片选中的轮次联动：1 = branches.json，>=2 = branches-round-n.json。
      const next = await postForm<ProdMrPayload>("/api/requirement/prod-mrs", { reqId: req.id, round: String(round) })
      setPayload(next)
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
  ...
  const chip = payload ? `${round > 1 ? `轮次 ${round} · ` : ""}${okCount}/${results.length} ready${noDiffCount ? ` · ${noDiffCount} 无差异` : ""}` : round > 1 ? `轮次 ${round} · test/uat → prod` : "test/uat → prod"
  return <section className="react-panel react-prod-mr-panel"><PanelHead kicker="Production MR" title="生产 MR" chip={chip} />
    <p className="react-muted">按代码差异卡片选中轮次的分支登记（{round > 1 ? <code>branches-round-{round}.json</code> : <code>branches.json</code>}）为每个应用的需求分支创建生产 MR；后端目标 <code>master</code>，前端目标 <code>production</code>。在代码差异卡片切换轮次后，本卡片生成的 MR 也按该轮次分支计算。<strong>仅创建合并请求，不会自动合入生产分支</strong>，合入需在 GitLab 上由组长审批后手动执行。生成前先比对需求分支与生产分支的差异，<strong>无差异的仓库自动跳过 MR</strong>。再次点击会重新扫描并复用已存在的 open MR。</p>
