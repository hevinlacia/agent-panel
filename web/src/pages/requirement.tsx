...
export function RequirementPage() {
  ...
  return <PageChrome icon={<GitBranch size={15} />} eyebrow="Requirement" title={req?.title || id || "Requirement"} description={req?.description || "需求详情、状态流转、技术方案、上线清单、业务背景、经验总结与关联 session。"} actions={<><a href="/projects"><ArrowLeft size={15} />返回需求列表</a>{req ? <a href="#technical-plan"><FileCode2 size={15} />技术方案</a> : null}{req && !isOnlineIssue && !isSub ? <a href="#release-manifest"><AlertTriangle size={15} />上线清单</a> : null}{req ? <a href="#attachments"><Paperclip size={15} />附件</a> : null}{req ? <a href={`/requirement-doc?id=${encodeURIComponent(req.id)}&doc=background&title=${encodeURIComponent("业务背景文档")}`}><Library size={15} />业务背景</a> : null}{req ? <a href="#experience-summary"><Lightbulb size={15} />经验总结</a> : null}{req && !isOnlineIssue ? <a href="#code-review"><GitBranch size={15} />代码差异</a> : null}</>}>
    {error ? <ErrorCard error={error} /> : loading ? <LoadingCard /> : !req ? <EmptyCard>需求不存在：{id}</EmptyCard> : <div className="react-detail-grid">
      <section className="react-panel"><PanelHead kicker="Overview" title="需求信息" chip={<>{statusPill(effectiveStatus(req))}{groupBadge(req)}{experienceSummaryPill(req)}</>} /><div className="react-meta-grid"><span>Req ID <code>{req.id}</code></span><span>项目 {projectsOf(req)}</span><span>创建 {formatDate(req.createdAt)}</span><span>更新 {relAge(req.updatedAt)}</span><span>预计发版 {req.planRelease || "unknown"}{!isSub ? <button type="button" className="react-copy-link-btn" onClick={() => setEditField("planRelease")} title="修改预计发版">改</button> : null}</span><span>目录 {req.reqDir || "-"}</span><span>类别 {req.category || "需求"}{!isSub ? <button type="button" className="react-copy-link-btn" onClick={() => setEditField("category")} title="修改类别">改</button> : null}</span><span>来源 {req.source ?? "产品推动"}{req.source === "开发推动" ? "（需测试场景文档）" : ""}<button type="button" className="react-copy-link-btn" onClick={() => setEditField("source")} title="修改推动方">改</button></span>{!isSub ? <span>ONES {onesRef ? (onesRef.url ? <a href={onesRef.url} target="_blank" rel="noopener noreferrer" title={onesRef.raw}>{onesRef.label}</a> : <code title={onesRef.raw}>{onesRef.label}</code>) : <em className="react-muted">未关联</em>}<button type="button" className="react-copy-link-btn" onClick={() => setOnesModal(true)} title="登记 / 修改 / 清除 ONES 关联">{onesRef ? "改" : "＋登记"}</button></span> : null}{req.memberOf?.length ? <span>所属组 {req.memberOf.map((g) => <a key={g} href={`/requirement?id=${encodeURIComponent(g)}`}>{g}</a>)}</span> : null}{req.isSubReq && req.parentReqId ? <span>父需求 <a href={`/requirement?id=${encodeURIComponent(req.parentReqId)}`}>{req.parentReqId}</a>（并行执行单元，合回父分支）</span> : null}</div><p className="react-detail-desc">{req.description || "暂无描述"}</p></section>
