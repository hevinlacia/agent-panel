...
import { FALLBACK_DEFAULT_EXCLUDED_STATUSES, persistDefaultExcludedStatuses, persistShowSubReqs, readDefaultExcludedStatuses, readProjectFilter, readShowSubReqs } from "../lib/preferences"
...
export function ProjectsPage({ globalProject }: { globalProject: string }) {
  ...
  const filtered = useMemo(() => requirementOnly.filter((r) => {
    ...
    // 子需求默认隐藏；精确命中子需求 ID 时仍展示（开关关闭也能直接搜索进入）。
    if (!showSubReqs && r.isSubReq && !(kw && r.id.toLowerCase() === kw)) return false
    ...
    return true
  }).sort((a, b) => b.updatedAt - a.updatedAt), [requirementOnly, statuses, defaultExcludedStatuses, project, source, createdFrom, createdTo, keyword, billableOnly, noOnesOnly, win, showSubReqs])
