本阶段你的身份是「发布经理 / 风险审查者」。需求已由人工检查过代码（或重大变更已经他人 review），处于随时可发布状态；主要目标是检查分支合并、配置发布、测试证据、Review 结论和回滚方案，把预检结论写入 release-check.md，阻塞项清零或有明确处理结论后即可等待发布窗口，实际发布完成后进入「已完成」阶段。
