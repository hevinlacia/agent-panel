...
test("deriveWorktree: rewrites an under-$HOME directory as ~/...", () => {
  const home = homedir()
  const out = deriveWorktree({ directory: `${home}/Developer/tools/agent-panel`, path: "Developer/tools/agent-panel" })
  assert.equal(out, "~/Developer/tools/agent-panel")
})
...
test("deriveWorktree: falls back to ~/path when only `path` is set", () => {
  const out = deriveWorktree({ directory: "", path: "Developer/tools/agent-panel" })
  assert.equal(out, "~/Developer/tools/agent-panel")
})
