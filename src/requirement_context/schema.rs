...
pub(crate) fn requirement_api_schema() -> Value {
    json!({
        ...
        "rules": [
            ...
            "代码审查门禁强制走 agent-panel-code-review skill：POST /api/requirement/review-materials 备料（发布就绪前默认全量、发布就绪起默认增量，mode 可覆盖）→ 按 skill 的四轮审查法深读（全量审查扩大阅读必要上下文代码，不只看 diff）→ 写 code-review-ai.md（顶部含 `Review Gate: PASS/BLOCKED` 与 `Source: agent-panel-code-review skill` 标记；问题分 P0 阻断/P1 严重/P2 一般/P3 优化，P0 非空必须 BLOCKED、P1 非空放行但门禁 ⚠ 警示；含「自测清单交叉评估」小节交叉核对边界/并发流量风险与测试结果）→ PUT annotations（代码问题备注）+ review-checklist；缺 skill 标记、缺备注、备注过期、缺交叉评估或 P0 非空都会被门禁拦截。",
